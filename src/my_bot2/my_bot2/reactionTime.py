...
class CameraYoloNode(Node):
    def __init__(self):
            ...
            for codec in ['MJPG', 'H264', 'YUYV']:
                ...
                if ret and frame is not None and frame.mean() > 5.0:
                    ...
                    self.get_logger().info(
                        f'✅ Camera opened: /dev/video{index} | codec: {codec}'
                    )
                    break
        ...
        self.cooldown_sec = 2.0  # change this to any value you want
        ...
        self.first_human_detect_time = None
        self.stop_sent_time = None
    ...
    def timer_callback(self):
        ...
        if self.frame_count % 2 == 0:
            ...
            if human_found and self.first_human_detect_time is None:
                self.first_human_detect_time = time.time()
    ...
    def _update_state(self, human_found: bool):
        ...
        if self.state == 'moving':
            if human_found:
                self.state = 'stopped'
                self.stop_time = now
                self.stop_sent_time = now
                self.stop_pub.publish(Bool(data=True))

                if self.first_human_detect_time is not None:
                    reaction_time = self.stop_sent_time - self.first_human_detect_time
                    self.get_logger().warn(
                        f'🚨 HUMAN DETECTED — Stopping robot! Reaction time: {reaction_time:.10f} s'
                    )
                else:
